...
const UserEntry = (props) => {
    ...
    const register = async (e)=>{
        e.preventDefault()
        setError({})
        await API().post(`https://medipathy.changetechbd.com/api/register?phone=${number}`)
        .then(res=>{
            console.log(res);
            if(res.data.data.id){
                setOTPActive(true)
    ...
    const login = async (e)=>{
        e.preventDefault()
        setError({})
        await API().post(`https://medipathy.changetechbd.com/api/login?phone=${number}&otp=${OTP}`)
        .then(res=>{
            console.log(res);
            if(res.data.token){
        ...
        <div className="user_entry pb-70">
            <div className="container-fluid">
                ...
                <div className="col-12 col-sm-12 col-md-12 col-lg-12">
                        ...
                        <div className="col-12 col-sm-12 col-md-12 col-lg-8 col-xl-6 offset-lg-2 offset-xl-3">
                            ...
                            <form action="#" onSubmit={!OTPActive?register : login}>
                                ...
                                <div className="col-12 col-sm-12 col-md-8 offset-lg-2 offset-xl-2">
                                    <input type="text" className="form-control input" id="number" placeholder="Enter your phone number" value={number} onChange={(e)=>setNumber(e.target.value)}  required />
                                    <div className="error-handler">{error.message}</div>
                                    <div className="error-handler">{error.numFormat}</div>
                                </div>
                                </div>
                                {OTPActive && <div className="form-group row align-items-center mb-2">
                                <div className="col-12 col-sm-12 col-md-8 offset-lg-2 offset-xl-2">
                                    <input type="text" className="form-control input" id="otp" placeholder="Enter your OTP" value={OTP} onChange={(e)=>setOTP(e.target.value)}  required />
                                    <div className="error-handler">{error.otpErrMessage}</div>
                                </div>
                                </div>}
                                <div className="login-box mt-3 text-center">
                                <button type="submit" className="btn mb-4 mt-2 col-12 col-sm-12 col-md-7">Continue</button>
                                </div>
                            </form>
                            </div>
                        </div>
                    ...
                    {error.otp && <div className="error-handler">Your OTP : {error.otp}</div>}
                    </div>
                </div> {/* end of row */}
